...
def parse_args():
    ...
    p.add_argument("--tests", nargs="*", default=None,
                   metavar="SAVR",
                   help="Space-separated list of tests to run, e.g. --tests 4 5 7 43_1. "
...
def main():
    ...
    start = start_d.strftime("%Y-%m-%d %H:%M:%S.000")
    ...
    cfg = json.loads(ROSTER.read_text())
    cfg["SAVR17"]["expected_agents"][0]["pid"] = int(python_pid)
    ROSTER.write_text(json.dumps(cfg, indent=2))
    print(f"[setup] SAVR17 roster patched with python PID {python_pid}")
    ...
    # 19. record end time and display total duration
    end_time = datetime.now()
    end_time_s = end_time.strftime("%Y-%m-%d %H:%M:%S.000")
    print(f"[setup] end timestamp: {end_time_s}")
    delta = (end_time - start_d)
    print(f"[setup] Total Duration: {delta}")
